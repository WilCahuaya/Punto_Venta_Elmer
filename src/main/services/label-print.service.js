...
import { buildLabelCellCss, buildLabelCellHtml, buildSingleLabelDocumentHtml, labelSizeClassName } from '@shared/lib/label-html';
import { A4_MARGIN_MM, A4_PAGE_HEIGHT_MM, A4_PAGE_WIDTH_MM, packA4Labels, isCompactLabel, labelBarcodeBarsMaxMm, labelBarcodeMaxWidthMm, mmToMicrons } from '@shared/lib/thermal-print';
...
function prepareBarcodeImage(sourcePath, dims, hasPrice) {
    ...
    const resized = targetW !== srcW || targetH !== srcH
...
export function isPdfVirtualPrinter(name) {
...
function buildLabelBodyHtml(content, barcode, dims) {
...
function buildLabelsDocumentHtml(bodies, dims) {
...
export function buildLabelHtml(content, barcode, dims) {
...
async function savePdfWithDialog(pdf, defaultName) {
...
async function printPdfFileToPrinter(pdfPath, printerName) {
    const script = `
    ...
    await new Promise((resolve, reject) => {
        ...
        proc.on('close', (code) => {
            if (code === 0)
                ...
                reject(new Error(stderr.trim() || `No se pudo imprimir el PDF (código ${code})`));
        });
    });
}
async function renderHtmlToPdf(htmlFile, page, windowSize) {
...
async function printHtmlSilent(htmlFile, page, device, dpi = 203) {
...
async function deliverLabelDocument(htmlFile, page, device, pdfDefaultName, dpi = 203) {
...
function cleanupBarcodeTemps(barcodes, originals) {
...
export async function printLabels(contents, printerName, dims, alternatePrinter = '') {
    if (!contents.length)
    ...
    const bodies = prepared.map(({ content, barcode }) => buildLabelBodyHtml(content, barcode, dims));
    ...
    try {
        await deliverLabelDocument(htmlFile, { widthMm: dims.widthMm, heightMm: dims.heightMm }, device, pdfName, dims.dpi);
    }
    finally {
        try {
            unlinkSync(htmlFile);
        ...
        cleanupBarcodeTemps(prepared.map((p) => p.barcode), contents.map((c) => c.barcodeImagePath));
    }
}
function contentDims(content, fallback) {
    return content.dims ?? fallback;
}
function uniqueLabelDims(dimsList) {
    const map = new Map();
...
function buildA4PackedDocumentHtml(cells, placements, sheets) {
...
function prepareA4PackedHtml(contents, fallbackDims) {
    const prepared = contents.map((content) => {
        const dims = contentDims(content, fallbackDims);
        const barcode = prepareBarcodeImage(content.barcodeImagePath, dims, Boolean(content.priceText));
        return { content, barcode, dims };
    });
    const pack = packA4Labels(prepared.map((p) => ({ widthMm: p.dims.widthMm, heightMm: p.dims.heightMm })));
...
export async function generateLabelsPdf(contents, dims, mode) {
    if (!contents.length)
        throw new Error('No hay etiquetas para previsualizar');
    let htmlFile;
    let page;
    let sheets = 1;
    let prepared;
    if (mode === 'a4') {
        ...
        prepared = contents.map((content) => {
            ...
            const barcode = prepareBarcodeImage(content.barcodeImagePath, itemDims, Boolean(content.priceText));
        ...
        const bodies = prepared.map(({ content, barcode }) => buildLabelBodyHtml(content, barcode, contentDims(content, rollDims)));
...
export async function printLabelsOnA4(contents, printerName, dims) {
    if (!contents.length)
        throw new Error('No hay etiquetas para imprimir');
    if (!printerName.trim())
        throw new Error('Seleccione una impresora para A4');
    ...
    try {
        try {
            ...
            try {
                await printHtmlSilent(htmlFile, { widthMm: A4_PAGE_WIDTH_MM, heightMm: A4_PAGE_HEIGHT_MM }, device, 300);
            }
            finally {
                try {
                    unlinkSync(htmlFile);
                ...
                cleanupBarcodeTemps(packed.prepared.map((p) => p.barcode), contents.map((c) => c.barcodeImagePath));
            }
        }
    }
    finally {
...
export async function printLabel(content, printerName, dims, alternatePrinter = '') {
    await printLabels([content], printerName, dims, alternatePrinter);
}
export async function printLabel50x25(content, printerName) {
    await printLabel(content, printerName, { widthMm: 50, heightMm: 25, dpi: 203 });
}
/** Etiqueta de prueba (layout y tamaño configurado). */
export async function printTestLabel(companyName, printerName, dims, alternatePrinter = '') {
    const html = [
    ...
    const device = await resolvePrinterName(printerName, 'etiquetas', alternatePrinter);
    try {
        await deliverLabelDocument(htmlFile, { widthMm: dims.widthMm, heightMm: dims.heightMm }, device, `etiqueta-prueba-${dims.widthMm}x${dims.heightMm}mm.pdf`, dims.dpi);
    }
    finally {
        try {
            unlinkSync(htmlFile);
