...
import { parseThermalPaperSize } from './pos-print-options';
import { printThermalLines } from './thermal-print.service';
import { disposePreparedTicketLogo, escPosDotsToCssPx, getPaperAndLogoPercent, prepareTicketLogoForPrint } from './ticket-logo';
...
function getPrintContext() {
    return getPaperAndLogoPercent(getSetting('printer_paper_width', '58mm'), getSetting('ticket_logo_width_percent', '65'));
}
async function posPrint(data, printerName) {
...
function ticketWidth(paper) {
    return TICKET_CHARS[paper];
}
function separatorLine(paper) {
    return {
        type: 'text',
...
function appendSlogan(data, slogan, paper) {
...
function appendTicketLogo(data, prepared, paper) {
    if (!prepared)
        return;
...
function buildTicketHeader(companyName, companyAddress, logoPath, ctx) {
    const data = [];
    let prepared = null;
    ...
    data.push(centerText(companyName, { fontWeight: '700', fontSize: '14px' }));
    if (companyAddress) {
        data.push(centerText(companyAddress, { fontSize: '10px' }));
    }
    return { data, prepared };
}
export async function printSaleTicket(saleId, printerOverride) {
    const db = getDatabase();
    ...
    const fmt = (n) => formatMoney(fromMoneyDb(n), currencySymbol);
    ...
    try {
        ...
        for (const item of items) {
            ...
            data.push({
                type: 'text',
                value: padRow(`${item.quantity} x ${fmt(item.unit_price)}`, fmt(item.line_total), width),
                style: { fontSize: '10px' }
            });
        }
        data.push(separatorLine(ctx.paper), {
        ...
        data.push(separatorLine(ctx.paper), centerText('Gracias por su compra!', { fontSize: '11px' }));
...
export async function printTestTicket() {
    ...
    try {
        ...
        data.push(separatorLine(ctx.paper), centerText('TICKET DE PRUEBA', { fontWeight: '700', fontSize: '13px' }), centerText(now, { fontSize: '10px' }), separatorLine(ctx.paper), {
            type: 'text',
            ...
            style: { fontWeight: '700', fontSize: '13px' }
        }, centerText('Impresora configurada correctamente', { fontSize: '10px' }), centerText('Gracias por su compra!', { fontSize: '11px' }));
        appendSlogan(data, ticketSlogan, ctx.paper);
        const method = await posPrint(data, printerName);
        return { ok: true, method };
    }
