...
export function updateSettings(input) {
    ...
    if (input.theme !== undefined)
        upsertSetting(db, 'theme', input.theme);
    if (input.currencySymbol !== undefined)
        upsertSetting(db, 'currency_symbol', input.currencySymbol.trim());
    if (input.soundsEnabled !== undefined) {
        upsertSetting(db, 'sounds_enabled', input.soundsEnabled ? 'true' : 'false');
    }
    if (input.companyName !== undefined)
        upsertSetting(db, 'company_name', input.companyName.trim());
    if (input.companyAddress !== undefined)
        upsertSetting(db, 'company_address', input.companyAddress.trim());
    if (input.printerTicket !== undefined)
        upsertSetting(db, 'printer_ticket', input.printerTicket);
    if (input.printerLabels !== undefined)
        upsertSetting(db, 'printer_labels', input.printerLabels);
    if (input.printerPaperWidth !== undefined) {
        upsertSetting(db, 'printer_paper_width', input.printerPaperWidth === '80mm' ? '80mm' : '58mm');
    }
    ...
    if (input.ticketLogoWidthPercent !== undefined) {
        upsertSetting(db, 'ticket_logo_width_percent', String(parseTicketLogoWidthPercent(String(input.ticketLogoWidthPercent))));
    }
...
export async function pickCompanyLogoService() {
...
export function removeCompanyLogoService() {
    const db = getDatabase();
    const map = getSettingsMap(db);
...
export function getLogoUrlService(relativePath) {
    return { ok: true, data: getImageMediaUrl(relativePath) };
}
export async function testPrintLabelService() {
    const db = getDatabase();
    const map = getSettingsMap(db);
...
export async function testPrintTicketService() {
