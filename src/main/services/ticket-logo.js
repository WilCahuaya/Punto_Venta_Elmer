...
import { ESC_POS_PRINTABLE_WIDTH_DOTS, PAPER_WIDTH_PX, parseThermalPaperSize } from './pos-print-options';
export function parseTicketLogoWidthPercent(value) {
...
export function getTicketLogoWidthDots(paper, widthPercent) {
...
export function escPosDotsToCssPx(dots, paper) {
 ...
 */
export function prepareTicketLogoForPrint(absoluteLogoPath, paper, widthPercent) {
...
export function disposePreparedTicketLogo(prepared) {
    if (!prepared)
        return;
    try {
        if (existsSync(prepared.tempFile))
...
export function getPaperAndLogoPercent(paperSetting, percentSetting) {
    return {
