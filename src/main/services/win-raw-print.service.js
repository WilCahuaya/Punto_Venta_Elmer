...
export async function resolveWindowsQueueName(configuredName) {
...
`;
    ...
    if (!name)
        throw new Error(`Impresora no encontrada: ${wanted}`);
    return name;
}
/** Puerto físico (USB001, COM3, etc.). */
export async function getWindowsPrinterPort(queueName) {
    const script = `
 ...
 */
export async function sendRawBytesWinSpool(queueName, data) {
...
export async function sendRawBytesToPort(portName, data) {
...
export async function sendRawEscPosWindows(configuredPrinterName, data) {
    ...
    throw new Error(`No se pudo enviar ESC/POS a "${queueName}"${port ? ` (${port})` : ''}. ${errors.join(' | ')}`);
}
