...
function addColumnIfMissing(database, table, definition) {
    ...
    if (!column || columnNames(database, table).has(column))
        return;
    database.exec(`ALTER TABLE ${table} ADD COLUMN ${definition}`);
}
function migrateCategoriesAndProductFields(database) {
  ...
  `);
    ...
    database.exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_categories_parent_name
...
function migrateReturnsAndVoidAudit(database) {
...
function migrateLabelPrintHistory(database, sql) {
    const jobsCols = columnNames(database, 'label_print_jobs');
    const isLegacyJobs = tableExists(database, 'label_print_jobs') &&
...
function migrateLabelItemSize(database) {
    if (!tableExists(database, 'label_print_job_items'))
        return;
...
function migratePaymentMethod(database) {
...
function migrateCreditSales(database) {
...
function migrateProductPackPrices(database) {
