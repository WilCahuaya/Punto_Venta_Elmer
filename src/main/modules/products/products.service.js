...
import { deleteImageIfExists, getImageMediaUrl, pickImageFile, storeProductImage } from '../../services/image.service';
...
import { getProductByBarcode, getProductByBarcodeRow, getProductByCode, getProductById, insertProduct, listProducts, searchProductsPos, countSaleItemsForProduct, hardDeleteProduct, softDeleteProduct, updateProduct, updateProductImagePath, updateProductStock } from './products.repository';
...
function validateProductInput(input, isUpdate = false) {
    ...
    if (wholesale != null && wholesale < 0)
        return 'El precio por mayor no puede ser negativo';
    ...
    if (dozen < 0)
        return 'El precio por unidad de la docena no puede ser negativo';
    const planchaQty = input.planchaQty ?? 0;
    const pricePlancha = input.pricePlancha ?? 0;
    if (planchaQty < 0)
        return 'La cantidad de la plancha no puede ser negativa';
    if (pricePlancha < 0)
        return 'El precio por unidad de la plancha no puede ser negativo';
    ...
    if (cajonQty < 0)
        return 'La cantidad del cajón no puede ser negativa';
    if (priceCajon < 0)
        return 'El precio por unidad del cajón no puede ser negativo';
...
function buildProductData(input, imagePath, productCode) {
    const wholesale = input.priceWholesale != null && input.priceWholesale > 0
...
function handleImage(productId, currentPath, pendingImagePath, removeImage) {
    if (removeImage) {
...
export function listProductsService(filters = {}) {
...
export function lookupProductByBarcodeService(barcode) {
...
export function adjustStockService(input) {
    if (input.stock < 0)
...
export function updateProductService(id, input) {
...
export function deactivateProductService(id) {
    const db = getDatabase();
    const existing = getProductById(db, id);
    if (!existing)
        return { ok: false, error: 'Producto no encontrado' };
...
export function destroyProductService(id) {
    const db = getDatabase();
    const existing = getProductById(db, id);
    if (!existing)
        return { ok: false, error: 'Producto no encontrado' };
...
export async function pickProductImageService() {
    const path = await pickImageFile();
    return { ok: true, data: path };
}
export function getProductImageUrlService(relativePath) {
    return { ok: true, data: getImageMediaUrl(relativePath) };
}
export function searchProductsPosService(query) {
    if (!query.trim())
        return { ok: true, data: [] };
    const db = getDatabase();
...
export function getSystemServiceProductService() {
    const db = getDatabase();
